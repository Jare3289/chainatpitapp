...
try {
    ...
    $allExistingPKs    = [];   // id (PK) → {id, user_id, student_id, name, class_name}
    ...
    foreach ($pdo->query("SELECT id, student_id, user_id{$fnthSel}, first_name_th, last_name_th, class_name FROM students")->fetchAll(PDO::FETCH_ASSOC) as $row) {
        ...
        $allExistingPKs[$row['id']] = [
            'id'         => $row['id'],
            'user_id'    => $row['user_id'],
            'student_id' => $row['student_id'],
            'name'       => $nameKey ?: trim(($row['first_name_th'] ?? '') . ' ' . ($row['last_name_th'] ?? '')),
    ...
    $matchedDbPKs = [];   // DB `id` PKs that were touched (updated or inserted)
    ...
    foreach ($rows as $idx => $rawData) {
        ...
        if ($existing) {
            ...
            $pdo->prepare("INSERT INTO students (`" . implode('`,`', $cols) . "`) VALUES (" . implode(',', array_fill(0, count($cols), '?')) . ")")
                ->execute($vals);
            $matchedDbPKs[(int)$pdo->lastInsertId()] = true;
            $insertCount++;
        }
        $sinceLastCommit++;
    ...
    // ── Sync-delete: remove students not present in the uploaded file ──
    $toDeletePKs   = array_diff_key($allExistingPKs, $matchedDbPKs);
    $deleteCount      = 0;
    $deleteFailCount  = 0;
    $willDelete       = [];

    if ($dryRun) {
        $willDelete  = array_values($toDeletePKs);
        $deleteCount = count($willDelete);
    } else {
        foreach ($toDeletePKs as $pk => $info) {
            $userId = $info['user_id'];
            try {
                $pdo->beginTransaction();
                $pdo->prepare("DELETE FROM students WHERE id = ?")->execute([$pk]);
                if ($userId) {
                ...
                // FK RESTRICT (errno 1451) — student has attendance/credit records
    ...
    echo json_encode([
        ...
        'will_delete'     => $dryRun ? $willDelete : [],
        'skip_reasons'    => array_slice($skipReasons, 0, 20),
        'message'         => $dryRun
            ? "ตรวจสอบแล้ว: เพิ่มใหม่ $insertCount, อัปเดต $updateCount, จะลบ $deleteCount รายการ"
            : "เพิ่มใหม่ $insertCount, อัปเดต $updateCount, ลบ $deleteCount, ข้าม $skipCount รายการ" . ($deleteFailCount ? " (ลบไม่ได้ $deleteFailCount รายการ — มีข้อมูลผูกพัน)" : ''),
